...
private:
	...
	void DFS(unordered_map<string, unordered_set<string>>& um, string preword, const string& endWord, int length, vector<string> prewords, const int& correct_length)
	{
		unordered_set<string> next = um[preword];
		...
		else
		{
			...
			{
				prewords.push_back(*iter);
				DFS(um, *iter, endWord, length + 1, prewords, correct_length);
				prewords.pop_back();
			}
		}
	}
...
public:
	vector<vector<string>> findLadders(string beginWord, string endWord, vector<string>& wordList) {
		...
		if (isconvert(beginWord, endWord))
		{
			res.push_back({ beginWord, endWord });
			return res;
		}
		...
		//if (find(wordList.begin(), wordList.end(), beginWord) == wordList.end()) wordset.erase(beginWord);
		...
		while (!breakflag)
		{
			...
			if (!isreverse)
			{
				...
				if (history.empty()) return vector<vector<string>> {}; //强制退出,无转换序列
				for (auto phstring = history.begin(); phstring != history.end(); ++phstring)
				{
					//for(set<int>::iterator it=wordset.begin() ;it!=wordset.end();)
					for (auto pnextstring = wordset.begin(); pnextstring != wordset.end(); ++pnextstring)
					{
						if (isconvert(*phstring, *pnextstring))
						{
							um[*phstring].insert(*pnextstring);
							nextset.insert(*pnextstring);
							if (history_reverse.find(*pnextstring) != history_reverse.end())
							{
								breakflag = true;
							}
							//pnextstring = wordset.erase(pnextstring);
						}
					}
				}
				history = nextset;
			...
			else
			{
				...
				if (history_reverse.empty()) return vector<vector<string>> {}; //强制退出,无转换序列
				for (auto phstring = history_reverse.begin(); phstring != history_reverse.end(); ++phstring)
				{
					//for(set<int>::iterator it=wordset.begin() ;it!=wordset.end();)
					for (auto pnextstring = wordset_reverse.begin(); pnextstring != wordset_reverse.end(); ++pnextstring)
					{
						if (isconvert(*phstring, *pnextstring))
						{
							um[*pnextstring].insert(*phstring);
							nextset.insert(*pnextstring);
							if (history.find(*pnextstring) != history.end())
							{
								breakflag = true;
							}
							//pnextstring = wordset.erase(pnextstring);
		...
		//DFS获取转换序列
		DFS(um, beginWord, endWord, 1, { beginWord }, length);
...
int main()
{
	...
	vector<vector<string>> res = s->findLadders("charge", "comedo", wordList);
